<template>
    <article v-if="add"class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative z-10 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <!-- Body -->
             <form action="" @submit.prevent="addEdu" class="flex flex-col gap-1">
                ...
                <button v-if="add" type="submit" class="border text-blue-500 bg-blue-500/20">Add</button>
                <!-- <button v-else="edit" type="submit" class="border text-yellow-500 bg-yellow-500/20">Edit</button> -->
            </form>
        </div>
    </article>
    <article v-else-if="edit"class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/50" @click="closeModal"></div>
        <div class="relative z-10 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <form action="" @submit.prevent="addEdu" class="flex flex-col gap-1">
                <label for="">name</label>
